...
export async function POST(request: NextRequest) {
  try {
    ...
    const data = await authenticatedApiRequest(`/users/${userId}/hustle-score`)
    return NextResponse.json(data)
  } catch (error) {
    console.error("Failed to fetch hustle score:", error)
    return NextResponse.json({ error: "Failed to fetch hustle score" }, { status: 500 })
  }
}
